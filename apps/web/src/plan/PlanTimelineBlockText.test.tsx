...
/** 10.1：「西湖」09:00 起 3 小时挂着 300 元、「灵隐寺」14:00 起 2 小时没挂钱、「在杭州」（停留）整天。 */
...
describe("块上写标题，还是标题加钱", () => {
  it("默认只写标题；开了以后横条上多写一行钱，这一行的道变高", async () => {
    ...
    await pressBlockText(user, "标题 + 钱");
  ...
  it("没挂钱的写淡色的「填钱」；垫在下面的细条不写钱", async () => {
    const user = userEvent.setup();
    await dayWithMoney();
    ...
    expect(empty.textContent).toBe("填钱");
  ...
  it("点块上的钱：选中这件事，就地改金额", async () => {
    ...
    expect(screen.getByRole("region", { name: "钱的总览" }).textContent).toContain("总额 ¥280");
  });
  ...
  it("选的记在这台设备上：切走再回来还是它，另一个计划还是只写标题", async () => {
    ...
    expect(within(group).getByRole("button", { name: "标题 + 钱" }).getAttribute("aria-pressed")).toBe("true");
