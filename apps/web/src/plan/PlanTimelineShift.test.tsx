...
/** 点一下 title 选中它，从快捷条的「详情…」开面板，再在面板里推迟 choice。 */
async function shiftFrom(user: User, container: HTMLElement, title: string, choice: string): Promise<void> {
  ...
  await user.click(within(within(panel).getByRole("group", { name: SHIFT })).getByRole("button", { name: choice }));
}
...
describe("推迟这天后面的安排", () => {
  it("从详情面板推迟 30 分钟：这件和后面的挪了，前面的不动；面板关掉", async () => {
  ...
  it("推过 24 点：换到下一行", async () => {
    ...
    expect(screen.queryByRole("dialog", { name: "夜游" })).toBeNull();
  });

  it("快捷条上没有推迟；面板里排上时间的有、没排时间的没有", async () => {
    const user = userEvent.setup();
    await openStoredPlan((plan, library) => {
      const [oct1] = daysFromOct1(plan, 1);
    ...
    expect(within(screen.getByRole("toolbar", { name: "「西湖」的操作" })).queryByRole("button", { name: SHIFT })).toBeNull();
    await user.click(screen.getByRole("button", { name: "详情…" }));
    const choices = within(within(screen.getByRole("dialog", { name: "西湖" })).getByRole("group", { name: SHIFT }))
      .getAllByRole("button")
...
describe("窄屏", () => {
  ...
  it("竖条选中后从屏幕底部快捷条的「详情…」进面板推迟", async () => {
    const user = userEvent.setup();
    await openStoredPlan((plan, library) => threeThings(plan, library));
