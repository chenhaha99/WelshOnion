...
import { newPlan } from "./timeline-helpers";
...
const NOT_TIMED = "排上时间的事会画在这里：把右边没排时间的事拖到时间轴上，或者在下面的安排表里点时间格";

test("电脑上新计划的第一屏：看得见「加第一件事」、没有「只看」→「24」不挨着「没排时间」→ 点了焦点到第 1 天 → 加一件换回原来那句 → 页顶「计划设置」", async ({
  page,
}) => {
...
test("手机上新计划的第一屏：「加第一件事」「计划设置」都在第一屏 → 点了第 1 天的「加一件事」拿到焦点、在屏幕里", async ({ page }) => {
  const errors = watchErrors(page);
