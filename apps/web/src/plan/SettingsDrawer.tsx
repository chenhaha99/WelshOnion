...
/**
 * 计划设置：电脑上从右边滑出，不盖住后面的内容；手机上（宽不到 720 像素）占满屏幕，露出的一条看不清还会误点。
 * 标题行钉在顶上，滚到下面也关得掉。每一栏回车或离开时保存。
 */
export function SettingsDrawer({ doc, library, settings, onClose }: SettingsDrawerProps) {
  ...
  return (
    <aside
      ...
      role="dialog"
      aria-label="计划设置"
      className="drawer fixed top-0 right-0 z-30 flex h-full w-full flex-col overflow-y-auto min-[720px]:w-80"
      onKeyDown={(event) => {
