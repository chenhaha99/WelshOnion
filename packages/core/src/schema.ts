 ...
 * 2 → 3（2026-09-20 标记从两档变三档）：checked 是 true 的写成 mark，其余不写（读出来就是「确定」），删掉 checked。
 * 3 → 4（同一天「划掉」改叫「完成」）：mark 是 "struck" 的改成 "done"。
 */
export function upgradePlanDoc(doc: Y.Doc): void {
  for (const block of doc.getMap<Y.Map<unknown>>("blocks").values()) {
    ...
    if (block.get("checked") === true) block.set("mark", "done");
    block.delete("checked");
    // 3 → 4：「划掉」改叫「完成」
    if (block.get("mark") === "struck") block.set("mark", "done");
  }
  doc.getMap("meta").set("schema", SCHEMA_VERSION);
}
