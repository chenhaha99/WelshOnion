...
test("钱：填第一笔 → 一块多笔 → 不属于任何一天 → 总览 → 撤销 → 只用键盘 → 手机", async ({ page }) => {
  ...
  // 只用键盘：灵隐寺的钱格回车打开，填 45 回车
  await rows.nth(2).getByRole("button", { name: "钱" }).focus();
  await page.keyboard.press("Enter");
  await expect(page.getByRole("group", { name: "灵隐寺 的钱" }).getByRole("textbox", { name: "新一笔的金额" })).toBeFocused();
  await page.keyboard.type("45");
  await page.keyboard.press("Enter");
  await expect(rows.nth(2).locator("[data-money-cell]")).toHaveText("¥45");
  await expect(overview.locator("[data-money-summary]")).toHaveText("总额 ¥503.50 · 人均 ¥503.50 · 已填 4 / 共 4 笔");
