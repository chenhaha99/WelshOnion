...
export function useNotifyDeleted(): (deleted: Deleted) => void {
  ...
  if (!notify) throw new Error("删完的提示只在计划页里用");
  return notify;
}

interface Shown extends Deleted {
...
/**
 * 删完的提示：屏幕底部一条「删掉了……」和「撤销」。8 秒后、之后又改了计划、撤销或重做就不显示；又删了别的换成新的。
 * 放提示的 status 区域一直在页面上，新出来的字读屏才会读。
 */
export function DeletedNotice({ undo, children }: { undo: UndoControls; children: ReactNode }) {
  const [shown, setShown] = useState<Shown | null>(null);
  const serial = useRef(0);
  const stackChanges = useRef(undo.stackChanges);
  stackChanges.current = undo.stackChanges;

  const notify = useCallback((deleted: Deleted) => {
    serial.current += 1;
    setShown({ ...deleted, stackChanges: stackChanges.current(), serial: serial.current });
  }, []);
